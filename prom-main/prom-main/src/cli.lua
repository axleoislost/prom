...
-- Prepend antispy check before obfuscating
local antispyCheck = table.concat({
    "local function antispy()",
    "    if iscclosure and not iscclosure(wait) or v0 then",
...
local source = antispyCheck .. table.concat(lines_from(sourceFile), "\n");
